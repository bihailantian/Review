...
public class LrcUtil {
	...
	OnLrcParseCallback lrcParseCallback;
    public static Vector<TimeLrc> lrclist;
    private boolean IsLyricExist = false;
    private int lastLine = 0;

    public LrcUtil(OnLrcParseCallback callback) {
		lrcParseCallback = callback;
    }
    ...
    public void ReadLRC(File f) {
        try {
            if (f == null || !f.exists()) {
                IsLyricExist = false;
                lrclist = null;
                //lrcParseCallback.setMiniLrc("歌词不存在");
            } else {
                //lrcParseCallback.setMiniLrc("歌词存在");
            ...
            for (int i = 1; i < lrclist.size(); i++) {
                if (current < lrclist.get(i).getTimePoint())
                    if (i == 1 || current >= lrclist.get(i - 1).getTimePoint()) {
                        ...
                        lrcParseCallback.setMiniLrc(lrcString);//调用activity里面的方法
                    }
            }
        }
    }
    ...
    /**
     * 解析String lrcString回调
     */
    public interface OnLrcParseCallback {
        /**
         * 设置歌词内容
         *
         * @param lrcString 歌词内容
         */
        void setMiniLrc(String lrcString);
    }
}
